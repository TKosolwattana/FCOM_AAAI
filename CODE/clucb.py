...
###################### NOTE ########################
# For CLUCB: set the communication threshold gammaU = 1
####################################################
results_dict = {}
rec_a_q = 4569
rec_a_c = 4569
...
for (al_q, al_c) in itertools.product(alpha_q, alpha_c):
    results_dict[(al_q, al_c)] = Fed_CLUCB(N_TRIAL=N_TRIAL, N_ARMS=N_ARMS, N_FEATURE=N_FEATURE, break_point = min_target_cregret, eta_1 = 0.3, eta_2 = 0.3, alpha_q =al_q, alpha_c = al_c, X=X_2, Y=Y_2.T, init_q=vec_q, init_c=vec_C,m=M, K = K, X_to_X_m=X_to_X_m, X_reshape=X_reshape, oracle=oracle_case1, gammaU=1, iterations=5)
